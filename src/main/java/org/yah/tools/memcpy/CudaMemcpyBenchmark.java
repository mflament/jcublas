...
public class CudaMemcpyBenchmark {
    ...
    private static void test(MemcpyKernelsSupport kernelsSupport, Pointer hostMemory, Pointer deviceMemory) {
        ...
        test(kernelsSupport, hostMemory, deviceMemory, javaMemory, false);
        test(kernelsSupport, hostMemory, deviceMemory, javaMemory, true);
    }

    private static void test(MemcpyKernelsSupport kernelsSupport, Pointer hostMemory, Pointer deviceMemory, long[] javaMemory, boolean zeroCopy) {
        if (!TEST)
            return;
        ...
        Pointer hostPtr = zeroCopy ? kernelsSupport.hostRegister(hostMemory, SIZE) : hostMemory;
        run(kernelsSupport, hostPtr, deviceMemory, zeroCopy);
        if (zeroCopy)
            kernelsSupport.hostUnregister(hostMemory);
        ...
        if (errors > 0)
            throw new IllegalStateException(label(zeroCopy) + ": " + errors + " errors");
    }

    private static void bench(MemcpyKernelsSupport kernelsSupport, Pointer hostMemory, Pointer deviceMemory) {
        if (RUNS == 0)
            return;
        bench(kernelsSupport, hostMemory, deviceMemory, false);
        bench(kernelsSupport, hostMemory, deviceMemory, true);
    }

    private static void bench(MemcpyKernelsSupport kernelsSupport, Pointer hostMemory, Pointer deviceMemory, boolean zeroCopy) {
        Pointer hostPtr = zeroCopy ? kernelsSupport.hostRegister(hostMemory, SIZE) : hostMemory;

        for (int i = 0; i < WARMUPS; i++) run(kernelsSupport, hostPtr, deviceMemory, zeroCopy);

        MemoryTimes memoryTimes = new MemoryTimes();
        for (int i = 0; i < RUNS; i++)  memoryTimes.add(run(kernelsSupport, hostPtr, deviceMemory, zeroCopy));

        long totalSize = SIZE / GB * RUNS;
        System.out.printf("%s host2device=%.3f GB/s device2host=%.3f GB/s%n", label(zeroCopy),
    ...
    private static MemoryTimes run(MemcpyKernelsSupport kernelsSupport, Pointer hostPtr, Pointer deviceMemory, boolean zeroCopy) {
        MemoryTimes memoryTimes = new MemoryTimes();

        long start = System.nanoTime();
        if (zeroCopy)
            kernelsSupport.memcpy(deviceMemory, hostPtr, LENGTH);
        else
        ...
        if (zeroCopy)
            kernelsSupport.memcpy(hostPtr, deviceMemory, LENGTH);
        else
    ...
    private static String label(boolean zeroCopy) {
        return zeroCopy ? "zeroCopy" : "cudaMemcpy";
    }
